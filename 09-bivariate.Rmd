...
- value of Sterling and the Euro in US\$ today at 12.00 GMT
...
1. $\int_{a_1}^{b_1} \int_{a_2}^{b_2} f(y_1, y_2) dy_1 dy_2
...
If $f(y_2 | y_1) = f_2(y_2)$, for all $y_2$,
...
```{example, bv2}
Suppose that $Y_1$ and $Y_2$ have joint pdf
\[f(y_1 , y_2) = 1⁄\pi, \quad \text{for $y_1^2 + y_2^2 < 1$.}\]
...
```{example}
Returning to Example \@ref(exm:bv1), we see that
$Y_1$ and $Y_2$ are independent, as
\[f(y_1, y_2) = \frac{1}{4} = \frac{1}{2} \times \frac{1}{2}
= f_1(y_1) f_2(y_2),\]
for $-1 < y_1 < 1$ and $-1 < y_2 < 1$.
```

```{example}
Returning to Example \@ref(exm:bv2), $Y_1$ and
...
\[f(y_1, y_2) = (2 \pi)^{-1} {\det(\bm \Sigma)}^{-1/2} \exp\{ - \frac{1}{2}(\bm y - \bm \mu)^T \bm \Sigma^{-1}(\bm y - \bm \mu)\},
...
\[f_1(y_1) = \int_{-\infty}^\infty f(y_1, y_2) dy_2,\]
which reduces to (**TODO: add reference to exercises?**)
...
(**TODO: add reference to exercises?**)
...
This means that
\[Y_1 | Y_2 = y_2 \sim N(\mu_1 + \rho \sigma_1(y_2 - \mu_2)/\sigma_2,
  \sigma_1^2 (1 - \rho)^2).\]
If $Y_1$ and $Y_2$ are uncorrelated ($\rho = 0$),
knowing the value of $Y_2$ does not change the distribution
of $Y_1$. However, if $Y_1$ and $Y_2$ are correlated ($\rho \not = 0$),
...
&= M_{Y_1, Y_2}(at, bt) \\
&= \exp((a \mu_1 + b \mu_2) t + \frac{1}{2} (a^2 \sigma_1^2 + 2 a b \rho \sigma_1\sigma_2 + b^2 \sigma_2^2) t^2,
\end{align*}
...
$X_1 = Y_1 + Y_2$ and $X_2 = Y_1 - Y_2$. Under what
conditions are $X_1$ and $X_2$ independent? 
Since $X_1$ and $X_2$ are normally distributed
[**TODO: need to show this jointly**], we 
...
In this chapter we have considered some jointly 
distributed random variables and
obtained marginal and conditional properties. We have derived some
properties of bivariate normal distributions.

The results, especially these to do with conditional pdfs, 
are particularly important
in statistical modelling (e.g. in MATH2010) where we use the 
value of one variable
to predict or explain the behaviour of another variable.

For example, in medicine we use a prognostic variable $Y_1$ to
model a surgical
outcome $Y_2$ for a patient.
In finance, we might use a credit score $Y_1$ to inform us about
a default outcome $Y_2$ for a loan customer.
In both cases, we are interested 
in $Y_2 | Y_1 = y_1$.
