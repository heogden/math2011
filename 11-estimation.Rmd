...
```{example, bernconsist, name = "Consistency of esimator of Bernoulli success probability"}
...
is that isf $T$ is an unbiased estimator of $\theta$, then
...
\[m_r^\prime = \mu_r^\prime(\tilde \theta_1, \tilde \theta_2, \ldots, \tilde \theta_k) \textrm{ for $r = 1, 2, \ldots$, k}.\]
...
differentiating and seeting to zero. We have
...
We have
\[\frac{d^2}{d\theta^2} = -\frac{\sum_{i=1}^n y_i}{\theta^2} - 
