...
\[\mu_3 = E\{[Y - E(Y]^3\} = E\{(Y - \mu)^3\},\] writing \(\mu = E(Y)\).
Expanding, we have
...
skewed (\(\gamma_1 = 2\)), with a high kurtosis (\(\gamma_2 = 4\)),
...
\[M_Y^{(r)}(0) =  \lim_{t \rightarrow 0}\sum_{k=0}^\infty \frac{t^{k} \mu_{k + r}^\prime}{k!} = \mu_r,\]
as required.
\EndKnitrBlock{proof}
...
\(\mu_4 = \kappa_4 + 3 \kappa_2^2 = 0 + 3 \sigma^2\), so the kurtosis is
...
minimum, and y\_\{(n)\}, the sample maximum. We define the corresponding
random variables \(Y_{(1)} = \min\{Y_1, \ldots, Y_n\}\) and
