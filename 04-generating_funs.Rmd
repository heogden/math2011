...
\begin{align*}
M_Y(t) &= E(e^{Yt}) = \sum_{y=0}^n e^{yt} \binom{n}{y} \theta^y (1 - \theta)^{n-y} \\
...
\begin{align*}
M_Y(t) &= E(e^{Yt}) = \int_{-\infty}^\infty e^{ty}  
...
$\mu_4 = \kappa_4 + 3 \kappa_2^2 = 0 + 3 \sigma^2$,
so the kurtosis is $\gamma_2 = 3$ for all normal random variables.

**TODO: link back to previous chapter**
```
...
Since the mgf characterises the distribution, we concude
...
Since the cgf characterises the distribution, we concude
